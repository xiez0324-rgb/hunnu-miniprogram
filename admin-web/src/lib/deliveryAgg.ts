...
function labelOf(group: DeliveryGroup): string {
  ...
  return `${d?.grade || ""} ${d?.subject || (group.parent?.nickname ? "家长咨询" : "需求单")}`.trim() || "未关联需求单";
}
...
export function groupDeliveries(rows: DeliveryRow[]): DeliveryGroup[] {
  ...
  for (const row of rows) {
    ...
    if (!g) {
      g = { demandId, demandLabel: "", demand: row.demand, parent: row.parent, items: [], total: 0, recommendedCount: 0, orderStatuses: [], latestTime: row.createTime };
      map.set(demandId, g);
    }
    g.items.push(row);
